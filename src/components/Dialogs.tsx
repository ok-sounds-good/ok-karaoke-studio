  ...
  return (
    ...
    >
      <p className="workflow-guide__intro">
        Work through these steps in order. Every editor stays in this window; system file pickers
        only appear when you choose a file or destination.
      </p>
      <ol className="workflow-guide" aria-label="Primary karaoke workflow">
        <li>
          ...
          <div>
            ...
            <p>
              Begin blank, or reopen an editable <code>.oks</code> project.
            </p>
          </div>
          <div className="workflow-guide__actions">
            <Button size="sm" variant="ghost" onClick={onOpen}>
              <FolderOpen size={13} /> Open .oks
            </Button>
        ...
        <li>
          ...
          <div>
            <strong>Attach the backing track</strong>
            <p>Link MP3, WAV, M4A, FLAC, AAC, or OGG audio from your computer.</p>
          </div>
          <Button size="sm" variant="secondary" onClick={onAttachAudio}>
            <FileAudio2 size={13} /> Attach audio
          </Button>
        </li>
        <li>
          <span>
          ...
          <div>
            <strong>Add the lyrics</strong>
            <p>Paste one lyric line per row, or import an existing LRC into the active voice.</p>
          </div>
          <div className="workflow-guide__actions">
            <Button size="sm" variant="ghost" onClick={onImportLrc}>
              <Import size={13} /> Import LRC
            </Button>
            <Button size="sm" variant="secondary" onClick={onEditLyrics}>
              <Edit3 size={13} /> Edit lyrics
            </Button>
          </div>
        </li>
        <li>
        ...
        <li>
          ...
          <div>
            <strong>Correct timing in the Lyric Timing area</strong>
            <p>Select or drag word blocks to move them; drag either edge to resize.</p>
          </div>
          <Button size="sm" variant="secondary" onClick={onClose}>
            <MousePointer2 size={13} /> Show Lyric Timing
          </Button>
        </li>
        <li>
          <span>
          ...
          <div>
            <strong>Verify in Live Preview</strong>
            <p>
              Sync focus keeps timing lightweight; exit sync to review the configured Clear or
              Scroll stage view.
            </p>
          </div>
          <Button size="sm" variant="secondary" onClick={onClose}>
            <Play size={13} /> Show preview
          </Button>
        </li>
        <li>
          <span>
          ...
          <div>
            ...
            <p>
              Save the editable <code>.oks</code>, then export LRC, ASS, or a finished MP4.
            </p>
          </div>
          <div className="workflow-guide__actions">
            <Button size="sm" variant="ghost" onClick={onSave}>
              <Save size={13} /> Save .oks
            </Button>
            <Button size="sm" variant="secondary" onClick={onExport}>
              <Download size={13} /> Choose export
            </Button>
          </div>
        </li>
      </ol>
        ...
        <div className="line-fit-preview">
          ...
          <div className="fit-stage">
            ...
            <div>
              ...
              <p>
                {(lines.find((line) => line.trim()) || 'Your lyrics will appear here').replaceAll(
                  '/',
                  '·',
                )}
              </p>
  ...
  return (
    ...
    >
      {cancelConfirmationOpen ? (
        <div
          className="video-export-cancel-confirmation"
          role="alertdialog"
          aria-labelledby="video-export-cancel-title"
        >
          <CircleStop size={28} />
          <h3 id="video-export-cancel-title">Cancel video export?</h3>
          <p>
            The encoder will stop. Any partial MP4 will remain beside the destination you chose.
          </p>
          <div>
            <Button
              autoFocus
        ...
        <>
          <div
            className={`export-readiness ${!hasLyrics || issueCount ? 'export-readiness--warning' : ''}`}
          >
              ...
              <span>
                {!hasLyrics
                  ...
                  : issueCount
                    ...
                    : 'This project is ready to hand off.'}
              </span>
            </div>
          </div>
          {videoProgress && (
          ...
          {cancelError && (
            <p className="video-export-cancel-error" role="alert">
              {cancelError}
            </p>
          )}
          ...
          <div className="export-options">
            <button onClick={onExportLrc} disabled={exportingVideo || !activeTrackHasLyrics}>
              ...
              <span>
                ...
                <small>
                  {activeTrackName} ·{' '}
                  {activeTrackHasLyrics ? 'line and word timing' : 'add lyrics to this track first'}
                </small>
              </span>
              <Download size={16} />
            </button>
            <button onClick={onExportAss} disabled={exportingVideo || !hasLyrics}>
              ...
              <span>
                <strong>ASS karaoke subtitles</strong>
                <small>Lead vocal · karaoke timing tags</small>
              </span>
              <Download size={16} />
            </button>
            <button
              onClick={
                exportingVideo ? requestCancellation : () => onExportVideo({ resolution, fps })
              }
            ...
            <button onClick={onExportProject} disabled={exportingVideo}>
              ...
              <span>
                <strong>Editable .oks project</strong>
                <small>Portable project with lyrics, tracks, and timing</small>
              </span>
              <Download size={16} />
            </button>
          </div>
