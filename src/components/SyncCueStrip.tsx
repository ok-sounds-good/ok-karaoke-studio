...
export function SyncCueStrip({ track, syncCursor, onEditLyrics }: SyncCueStripProps) {
  ...
  const visibleLines = currentLineIndex >= 0
    ? [
    ...
    <section className="sync-cue panel" aria-label="Synchronization focus">
      <header className="panel-header sync-cue__header">
        <div className="panel-title">
          <span className="panel-title__icon"><Radio size={16} /></span>
          <div>
        ...
        <div className="sync-cue__actions">
          <span><Zap size={12} /> Word {Math.min(syncCursor + 1, words.length)} of {words.length}</span>
      ...
      <footer className="sync-cue__help">
        ...
        <span>Press at each word onset. The next press closes the previous word; hold the final word of a line to extend it.</span>
      </footer>
    </section>
  )
}
