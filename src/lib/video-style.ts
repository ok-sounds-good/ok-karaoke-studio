...
/**
 * The persisted stage defaults use the product's purple/orange visual identity.
 * Keep the palette in one place so new projects, persistence, Preview, and MP4
 * rendering cannot silently acquire different default colors.
 */
...
export function deterministicFontFamily(typeface: FontTypefaceDescriptor, alias?: string): string {
  if (typeface.kind === 'system-monospace') return 'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace'
  if (typeface.kind === 'system-ui') return 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'
  return alias
...
export function fontTypefaceKey(typeface: FontTypefaceDescriptor): string {
  const localIdentity = typeface.faces.map((face) => face.postscriptName).filter(Boolean).sort().join('|')
  return `${typeface.kind}:${typeface.family}:${localIdentity}`
}
...
export function resolveFontFace(
  ...
  const exactStyle = typeface.faces.find((face) => (
    face.style.toLocaleLowerCase() === requested.style.toLocaleLowerCase() &&
...
export function backgroundReadiness(
  ...
  if (!resolvedUrl) {
    return {
      ready: false,
      reason: resolutionError || `Linked background image is missing or unreadable: ${background.imagePath}`,
    }
  }
...
export function isValidSyncAid(style: VocalStyle): boolean {
  ...
  return Number.isSafeInteger(previewMs) &&
