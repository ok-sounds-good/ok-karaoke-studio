...
function decodeProfileIdentity(rawIdentity, code) {
  try {
    ...
    if (
      ...
      typeof parsed.dev !== 'string' || !parsed.dev ||
      typeof parsed.ino !== 'string' || !parsed.ino ||
...
function pathsAreSeparate(left, right) {
...
function readOwnerSync(profilePath, code) {
  ...
  const ownerStats = fsSync.lstatSync(ownerPath)
...
function validateOwnedSmokeProfile(rawPath, defaultUserDataPath, rawIdentity, code) {
  ...
  try {
    const stats = fsSync.lstatSync(profilePath)
...
async function createOwnedSmokeProfile(prefix, options = {}) {
  ...
  const stats = await fsApi.lstat(profilePath)
...
async function ownedProfileStillMatches(profile, fsApi) {
  try {
    const stats = await fsApi.lstat(profile.path)
    ...
    const ownerStats = await fsApi.lstat(ownerPath)
