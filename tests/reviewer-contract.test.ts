...
describe('reviewer infrastructure contract', () => {
  ...
  it('requires protected CircleCI checks without manufacturing a pass', () => {
    ...
    expect(pullRequestTemplate).toContain('Protected `ci/circleci: macOS` check and evidence')
    expect(pullRequestTemplate).toContain('Protected `ci/circleci: Windows` check and evidence')
